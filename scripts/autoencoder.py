...
def main():
    ...
    for col in df.columns.values:
        column_modes = df.loc[df[col] != '-', col].mode()[0]
        df[col] = df[col].replace('-', column_modes)
    ...
    history = autoencoder.fit(X_train, X_train, epochs=20, batch_size=32, shuffle=True, verbose=1)
    ...
    try:
        aegis_fused_df.to_excel("results/aegis_mc_markov_fused_hotspots.xlsx", index=False)
    except Exception:
    ...
    plt.tight_layout(); plt.savefig("results/entropy_strip.png")
    ...
    plt.tight_layout(); plt.savefig("results/epistemic_var_strip.png")
    ...
    plt.tight_layout(); plt.savefig("results/top_hotspots_mc.png")
    ...
    plt.tight_layout(); plt.savefig("results/markov_transition_avg.png");
    ...
    # 1) Load
    df = pd.read_excel("results/top_mutations_mc.xlsx")
    ...
    plt.tight_layout(); plt.savefig("results/top_mutations_confidence.png")
    ...
    plt.tight_layout(); plt.savefig("results/top_mutations_small_margin.png")
    ...
    plt.tight_layout(); plt.savefig("results/scatter_confidence_vs_position.png")
    ...
    plt.legend()
    plt.tight_layout(); plt.savefig("results/top1_top2_probs_ambiguous.png")
